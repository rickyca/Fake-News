...
# Pipeline
pl = make_pipeline(CountVectorizer(), BaggingClassifier())
print "Fitting..."

# Gridsearch params
params = {
    'baggingclassifier__base_estimator': [LogisticRegression(), KNeighborsClassifier(), SVC(), MLPClassifier()],
    'baggingclassifier__n_estimators': range(3,100),
    'baggingclassifier__max_features': np.arange(0.6,1.0,0.1) ,
    'countvectorizer__stop_words' : [None, 'english'],
    'countvectorizer__ngram_range': [(i,j) for j in range(1,4) for i in range(j,3)],
    'countvectorizer__max_features': range(10000,100000,2)
}
